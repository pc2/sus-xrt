...
- Creating the `ila_crc_counter` IP core. 
- Wiring it up (optionally in [SUS](https://sus-lang.org))
- Adding the IP core to `pack_kernel.tcl`
- Synthesize your design. (Your kernel can be instantiated multiple times, Vivado automatically finds ILA cores and )
...
## Part 1: Synthesizing
### Create the `ila_crc_counter` IP core
`create_ila.tcl`:
```tcl
# U280
...
Execute `create_ila.tcl` like so:
```sh
mkdir -p ip_creation
rm -rf ip_creation/ila_crc_counter
vivado -mode batch -source create_ila.tcl
```
...
Start by creating an `extern module` for the ILA:
```sus
...
### Adding the IP core to `pack_kernel.tcl`
This is rather simple, import the packed `./ip_creation/ila_crc_counter/ila_crc_counter.xci` into your kernel:
`pack_kernel.tcl`:
```
